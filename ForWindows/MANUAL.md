# ツイキャス コメントヘルパー 使い方ガイド(Windows版)
...
`twitcasting_helper.exe` をダブルクリックします。

初回だけ「WindowsによってPCが保護されました」という青い画面が出ることがあります。
その場合は次の手順で進めてください。
...
![起動直後の画面](manual_images/windows/01_live.png)
...
過去コメントもさかのぼって記録し、**ライブのコメントと時系列で混ぜてコメント欄にも
表示します**(はじめから付いています)。取得中は画面下部に進み具合が表示されます。
通信が不安定でも自動でやり直すので、そのままお待ちください。

- 接続するたびにコメント欄は一度空になり、その配信のコメントを最初から表示し直します。
  前に見ていた配信のコメントが残ったまま新しい配信と混ざることはありません。
- 過去コメントの取得中は、まだ全部が記録し終わっていない状態で連番が仮に表示される
  ことがあります。取得が完了すると自動で正しい番号に直ります(→ 次項)。

集計タブでは、DBに貯まっている分だけを対象に、発言者ごとのコメント数やキリ番を
踏んだ人を一覧で確認できます。

![集計タブ](manual_images/windows/02_aggregate.png)

### コメントの順番(連番)とキリ番のお知らせ

各コメントの通し番号(`#123`)は、**今見ている配信の最初のコメントを1番として**、
コメント行の**右端に寄せて**表示されます。日をまたいで配信しても、配信者を切り替えても、
接続するたびにその配信の頭から数え直されます。上のバーの「連番/キリ番」ボタンから、
「100人目」のようにお知らせしたい番号を自由に設定できます(この設定は連番の始点には
影響しません)。該当する番号のコメントが来ると、目立つ色で強調表示されます。
...
![色設定タブ](manual_images/windows/03_color.png)
...
![通知タブ](manual_images/windows/04_notify.png)
...
> 通知音を鳴らすには **miniaudio** というモジュールが必要です。配布されている `.exe`
...
![クイズタブ](manual_images/windows/05_quiz.png)
...
1. ブラウザで [VLC公式サイト](https://www.videolan.org/vlc/download-windows.html) を開きます。
2. 「Download VLC」を選んでダウンロードします。
3. ダウンロードしたファイルをダブルクリックし、画面の指示に従って
   「次へ」を選んでいけばインストール完了です。
...
ウィンドウ右上の「×」ボタンで閉じてください。作業中の内容は安全に保存されます。

---

## 10. 困ったときは

**Q. 「WindowsによってPCが保護されました」と出て開けない**
A. 画面の「詳細情報」→「実行」の順に選んでください。一度実行すると次回からは出ません。
...
- 保存場所: `%LOCALAPPDATA%\twitcasting_helper\`(エクスプローラーのアドレス欄に
  そのまま貼り付けると開けます)。
